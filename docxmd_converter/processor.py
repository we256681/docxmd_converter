...
class BaseDocumentProcessor:
    ...
    def is_already_processed(self, content: str) -> bool:
        ...
        return "<!-- METADATA" in content and '"document_type": "структурированный_документ"' in content

    def add_metadata(self, content: str, position: str, department: str, processing_quality: str = "medium") -> str:
    ...
    def assess_processing_quality(self, content: str) -> str:
        ...
        sections = ['Общие положения', 'Функции', 'Должностные обязанности', 'Права', 'Ответственность']
        filled_sections = 0

        for section in sections:
            if section in content and len(re.findall(f'{section}.*?(?=##|$)', content, re.DOTALL)) > 0:
                section_content = re.findall(f'{section}.*?(?=##|$)', content, re.DOTALL)[0]
...
class BasicDocumentProcessor(BaseDocumentProcessor):
    ...
    def process_file(self, file_path: Union[str, Path], force: bool = False) -> bool:
        ...
        try:
            ...
            final_content = self.add_metadata(structured_content, position, department, quality)
    ...
    def structure_content(self, content: str, filename: str) -> str:
        ...
        structured_content += "**Утверждено:** _(подпись, инициалы, фамилия руководителя)_\n\n"
        ...
        if sections.get('responsibility'):
            ...
            structured_content += self.format_responsibility_section(sections['responsibility'])
        else:
        ...
        structured_content += "**Подпись сотрудника:** _________________ _(инициалы, фамилия)_\n\n"
        structured_content += "**Дата:** _(дата)_\n\n"
    ...
    def extract_sections(self, content: str) -> Dict[str, str]:
        ...
        # General provisions
        general_match = re.search(r'1\.\s*Общие положения(.*?)(?=\d+\.|$)', content, re.DOTALL | re.IGNORECASE)
        if general_match:
            sections['general'] = general_match.group(1).strip()

        # Functions
        functions_match = re.search(r'2\.\s*Функции(.*?)(?=\d+\.|$)', content, re.DOTALL | re.IGNORECASE)
        if functions_match:
            sections['functions'] = functions_match.group(1).strip()

        # Duties
        duties_match = re.search(r'3\.\s*Должностные обязанности(.*?)(?=\d+\.|$)', content, re.DOTALL | re.IGNORECASE)
        if duties_match:
            sections['duties'] = duties_match.group(1).strip()

        # Rights
        rights_match = re.search(r'4\.\s*Права(.*?)(?=\d+\.|$)', content, re.DOTALL | re.IGNORECASE)
        if rights_match:
            sections['rights'] = rights_match.group(1).strip()

        # Responsibility
        resp_match = re.search(r'5\.\s*Ответственность(.*?)(?=\d+\.|─|$)', content, re.DOTALL | re.IGNORECASE)
        if resp_match:
...
class AdvancedDocumentProcessor(BaseDocumentProcessor):
    ...
    def process_file(self, file_path: Union[str, Path], force: bool = False) -> bool:
        ...
        try:
            ...
            # Check if already processed
            if self.is_already_processed(content) and not (force or self.force_reprocess):
                self.results.skipped += 1
                self.results.files_skipped.append(str(file_path))
                return True
            ...
            structured_content = self.advanced_structure_content(content, file_path.name)
