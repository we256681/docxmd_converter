...
class DocumentProcessor:
    ...
    def extract_position_name(self, content: str) -> str:
        ...
        if match:
            ...
            position = re.sub(r'(должностная инструкция|агент.*)', '', position, flags=re.IGNORECASE)
    ...
    def format_rights_section(self, content: str) -> str:
        ...
        for i, right in enumerate(rights, 1):
            ...
            if right and not re.match(r'_{5,}', right):  # Исключаем строки подчеркиваний
                result += f"{i}. {right}\n\n"
    ...
    def format_responsibility_section(self, content: str) -> str:
        ...
        for item in resp_items:
            ...
            if item and len(item) > 10 and not re.match(r'5\.\d', item):  # Исключаем номера и короткие строки
                result += f"- {item}\n\n"
    ...
    def normalize_department_name(self, path: str) -> str:
        ...
        parts = path.split('/')
        for part in parts:
            if part.isupper() and len(part) > 5:  # Находим название отдела в верхнем регистре
    ...
    def is_already_processed(self, content: str) -> bool:
        ...
        return "<!-- METADATA" in content and '"document_type": "должностная_инструкция"' in content
